...
const ProgressBar = ({ totalAmount }) => {
  ...
  const thresholds = [
    ...
    { amount: 2000, label: 'Free Gift', icon: <FaGift /> },
    { amount: 3000, label: 'Free Shipping', icon: <FaShippingFast /> },
  ];
  ...
  return (
    <div className='my-8 relative px-4'>
      ...
      <div className="progress-bar bg-gray-200 w-full h-2 relative rounded-full overflow-hidden">
        ...
        {/* Render circles for thresholds */}
      </div>
      <div className="thresholds absolute top-[-1rem] left-0 w-[90%] flex justify-between ">
        {thresholds.map((threshold, index) => (
          <div key={index} className={`threshold text-center flex justify-center items-center flex-col w-auto relative ${totalAmount >= threshold.amount && index !== 0 ? 'text-black-500' : 'text-black-400'}`}>
            {index !== 0 && (
